...
def vis_detections(gifnum, image_name, im, class_name, dets, thresh=0.5):
    ...
    for i in inds:
        ...
        text = '{}%:{}'.format(int(score*100),class_name)
        #cv2.rectangle(im, (int(bbox[0]),int(bbox[1]-20)), (int(bbox[0]+80),int(bbox[1])), (255,255,255), -1)
        cv2.putText(im,text,(int(bbox[0]),int(bbox[1]-5)),cv2.FONT_HERSHEY_COMPLEX, 0.7,(0,0,250),2)
        cv2.rectangle(im,(int(bbox[0]), int(bbox[1])),(int(bbox[2]),int(bbox[3])),(0,0,250),3)
...
def demo(net, image_name, gifnum):
    ...
    for cls_ind, cls in enumerate(CLASSES[1:]):
        ...
        dets = dets[keep, :]
        vis_detections(gifnum, image_name, im, cls, dets, thresh=CONF_THRESH)
